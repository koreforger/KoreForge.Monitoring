...
function Invoke-KoreForgeTest {
    ...
    try {
        ...
        # MonitoringRegistry.Tests uses WebApplicationFactory<Program> via PackageReference.
        # Set the SUT content root to the test output directory where appsettings.json is copied.
        $buildRoot = Get-KoreForgeRepoArtifactPath -RepoRoot $repoRoot -ChildPath 'build'
        $env:ASPNETCORE_TEST_CONTENTROOT_MONITORINGREGISTRY = Join-Path $buildRoot "bin\MonitoringRegistry.Tests\$Configuration"
